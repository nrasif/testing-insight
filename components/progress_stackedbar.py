...
def plot_data(df_selected):
    """
    Generate a stacked bar chart for UAT progress.
    ...
    if df_selected.empty:
        ...
        # Compute "Other" category (remaining execution percentage)
        df_selected.loc[:, 'Other'] = (df_selected['Execution'] - df_selected['Passed'] - df_selected['Failed']).clip(lower=0)
        ...
        for category, color in zip(categories, colors):
            fig.add_trace(go.Bar(
                ...
                y=df_selected.get(category, pd.Series([0] * len(df_selected))),  # Avoid KeyError
                name=category,
                marker_color=color,
                text=df_selected.get(category, pd.Series()).apply(lambda x: f"{x:.2f}%" if pd.notna(x) else ""),
                textposition='inside',
                textfont=dict(size=14, color='black'),
                hoverinfo='text',
                hovertext=df_selected.get(category, pd.Series()).apply(lambda x: f'{category}: {x:.2f}%' if pd.notna(x) else "")
            ))

        # Add execution and target lines
...
def uatProgress_stacked(df, selected_os):
    """
    UAT Progress stacked bar chart component.
        
    """
    df_selected = df[df['OS'] == selected_os]

    if df_selected.empty:
        st.warning(f"No data available for {selected_os}.")
        return
